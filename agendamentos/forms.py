...
class PetForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'nome': forms.TextInput(attrs={'placeholder': 'Ex: Rex, Luna, Thor...', 'class': 'form-control'}),
            'tipo': forms.Select(attrs={'class': 'form-select'}),
            'raca': forms.TextInput(attrs={'placeholder': 'Ex: Labrador, Siames, SRD...', 'class': 'form-control'}),
            'idade': forms.NumberInput(attrs={'placeholder': 'Idade em anos', 'class': 'form-control', 'min': '0', 'max': '30'}),
            'observacao': forms.Textarea(attrs={'placeholder': 'Alguma observação importante sobre seu pet...', 'class': 'form-control', 'rows': 3}),
        }
        labels = {
            'nome': 'Nome do Pet',
            'tipo': 'Tipo de Animal',
...
class AgendamentoForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'bairro': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Bairro', 'id': 'bairro'}),
            'cidade': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Cidade', 'id': 'cidade'}),
            'estado': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'UF', 'id': 'estado', 'maxlength': '2'}),
            'complemento': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Complemento (opcional)', 'id': 'complemento'}),
    ...
    def clean_cep(self):
        cep = re.sub(r'[^0-9]', '', self.cleaned_data.get('cep', ''))
        if len(cep) != 8:
